...
// Same meta map used in the client area
...
export default async function GestaoPage() {
  ...
  const { data: empresas } = await supabase
    .from("empresas")
    .select("id, razao_social, nome_fantasia, cnpj, regime, ativo")
    .order("razao_social");

  // Get all client folders from the filesystem
  const folders = getAllClientFolders();

  // Build a combined view: for each empresa, check if there's a matching folder
  ...
  // Also detect orphan folders (folders without a matching empresa)
    ...
    <main className="min-h-screen bg-its-dark text-white">
      <header className="bg-its-darker border-b border-its-gray p-4 sticky top-0 z-10">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <Link href="/gestao/obrigacoes" className="text-xs text-gray-400 hover:text-its-green">
              Obriga{"\u00e7\u00f5"}es
            </Link>
            <Link href="/gestao/processos" className="text-xs text-gray-400 hover:text-its-green">
              Processos
            </Link>
            <Link href="/gestao/clientes" className="text-xs text-gray-400 hover:text-its-green">
              Clientes
            </Link>
      ...
      <section className="max-w-6xl mx-auto p-6">
        <div className="flex items-center justify-between mb-2">
          ...
          <Link href="/gestao/clientes/novo" className="btn-green text-sm">
            + Novo cliente
          </Link>
        </div>
        <p className="text-gray-400 mb-8">
          Sistemas detectados automaticamente a partir das pastas em <code className="text-xs bg-its-gray px-2 py-0.5 rounded">/sistemas/{"<slug>"}/</code>
        </p>

        {clientesComSistemas.length === 0 && orphanFolders.length === 0 ? (
          <div className="card p-12 text-center text-gray-500">
            Nenhum cliente cadastrado. Clique em <b className="text-its-green">Novo cliente</b> para começar.
          </div>
        ) : (
              ...
              <div key={c.id} className="card overflow-hidden">
                <div className="p-6 flex items-start justify-between">
                  <div>
                    <div className="flex items-center gap-3">
                      ...
                      <span className={`text-[10px] font-bold tracking-wider px-2 py-0.5 rounded ${
                        c.ativo ? "bg-its-green text-black" : "bg-gray-700 text-gray-400"
                      }`}>
                  ...
                  <div className="text-right text-xs text-gray-500">
                    {c.sistemas.length} sistema{c.sistemas.length !== 1 ? "s" : ""}
                  </div>
                </div>
                ...
                {c.sistemas.length > 0 ? (
                  <div className="border-t border-its-gray px-6 py-4">
                    <div className="flex flex-wrap gap-3">
                      {c.sistemas.map((s) => (
                        <div
                          key={s.slug}
                          className="flex items-center gap-2 bg-its-darker border border-its-gray rounded-lg px-4 py-2 text-sm"
                        >
                          <span>{s.icone}</span>
                  ...
                  <div className="border-t border-its-gray px-6 py-4 text-sm text-gray-500">
                    {c.hasFolder
                      ? "Pasta encontrada, mas sem arquivos .html"
                      : <>Sem pasta <code className="bg-its-gray px-1 rounded text-xs">/sistemas/{c.slug}/</code> — crie para adicionar sistemas</>
                    }
                  </div>
                )}
              </div>
            ...
            {orphanFolders.length > 0 && (
              <div className="card p-6 border-yellow-600/30">
                <div className="text-sm font-bold text-yellow-400 mb-2">
                  Pastas sem empresa vinculada
                </div>
                <div className="flex flex-wrap gap-2">
                  {orphanFolders.map(f => (
                    <code key={f} className="bg-its-gray px-2 py-1 rounded text-xs text-gray-400">
                      /sistemas/{f}/
                    </code>
                  ))}
                </div>
                <p className="text-xs text-gray-500 mt-2">
                  Cadastre uma empresa com nome/fantasia correspondente ou renomeie a pasta.
                </p>
              </div>
            )}
          </div>
