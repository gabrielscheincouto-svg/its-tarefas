      ...
      <section className="max-w-6xl mx-auto p-6 space-y-8">
              ...
              {documentos.map((doc) => (
                <div key={doc.id} className="flex items-center gap-3 bg-[#0a0a0a] border border-[#1f1f1f] rounded-lg px-4 py-3">
                  <span className="text-lg">
                    {doc.nome_arquivo.endsWith(".pdf") ? "\uD83D\uDCC4" : doc.nome_arquivo.endsWith(".xlsx") ? "\uD83D\uDCCA" : "\uD83D\uDCCE"}
                  </span>
          ...
          {!isPublished && (
            <div className="border-t border-[#1f1f1f] pt-4">
              ...
              <div className="grid md:grid-cols-4 gap-3">
                <input
                  placeholder="Item (ex: PIS/COFINS)"
                  value={newItem}
                  onChange={(e) => setNewItem(e.target.value)}
                  className="bg-[#0a0a0a] border border-[#1f1f1f] rounded-lg px-3 py-2 text-sm text-white focus:border-[#00E676] focus:outline-none"
                />
                <input
                  ...
                  step="0.01"
                  value={newVC}
                  onChange={(e) => setNewVC(e.target.value)}
                  className="bg-[#0a0a0a] border border-[#1f1f1f] rounded-lg px-3 py-2 text-sm text-white focus:border-[#00E676] focus:outline-none"
                />
                <input
                  placeholder="Valor ITS"
                  type="number"
                  step="0.01"
                  value={newVI}
              ...
              <button
                ...
                disabled={addingAudit || !newItem || !newVC || !newVI}
                className="mt-3 bg-[#1f1f1f] text-white font-bold py-2 px-6 rounded-xl hover:bg-[#2a2a2a] disabled:opacity-50 transition text-sm"
              >
        ...
        <div className="bg-[#141414] border border-[#1f1f1f] rounded-xl p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            {!isPublished && (
              <button
                onClick={saveIndicadores}
                disabled={savingInd}
                className="text-sm bg-[#1f1f1f] text-white font-bold py-1.5 px-4 rounded-lg hover:bg-[#2a2a2a] disabled:opacity-50 transition"
              >
